L.Control.Elevation = L.Control.extend({
    ...
    onAdd: function(map) {
        ...
        opts.hoverNumber.formatter_time = opts.hoverNumber.formatter_time || this._formatter_time;
		...
		var focusrectX = this._focusrectX = this._focusG.append("svg:rect")		
		var focusrectYZ = this._focusrectYZ = this._focusG.append("svg:rect")		
        ...
        this._focuslabelZ = focusG.append("svg:text")
            .style("pointer-events", "none")
            .attr("class", "mouse-focus-label-z");
    ...
    _findItemForLatLng: function(latlng) {
        ...
        this._data.forEach(function(item) {
            var dist_time = latlng.distanceTo(item.latlng);
            if (dist_time < d) {
                d = dist_time;
                result = item;
            }
...
/*
     * Fromatting funciton  for time
     *		
     */

    _formatter_time: function(num) {
		var res;
		res=num;
		heure=Math.floor(num);
		minute=(num-heure)*60
		minu=Math.floor(minute);
		var zero=""
		if (minu<10) zero=0
		sec=Math.floor((minute-minu)*60);
		res= heure + "h" + zero + minu + "'" + sec + "''";
        return res;
	},
            ...
            x.attr("class", "x axis")
                ...
                .call(d3.svg.axis()
                    ...
                    .tickFormat(function(d) {
												tr=d-Math.floor(d)
												if (Math.round(tr*60)<10) {zer="0"} else {zer=""}
												d=Math.floor(d)+ "h" + zer + Math.round(tr*60) + "'"
												return d 						
												})
					.orient("bottom"))
                .append("text")
...
if (opts.distance_time) {			
    ...
    _mousemoveHandler: function(d, i, ctx) {
        ...
        var item = this._data[this._findItemForX(coords[0])],
            alt = item.altitude,
            dist_time = item.dist_time,
            ll = item.latlng,
            numY = opts.hoverNumber.formatter(alt, opts.hoverNumber.decimalsY),
            numX = opts.hoverNumber.formatter(dist_time, opts.hoverNumber.decimalsX);
    ...
    _addGPXdata: function(coords) {
        ...
        if (coords) {
            ...
            for (var i = 0; i < coords.length; i++) {
                ...
                if (i == 0) {var time_deb =s.meta.time };
				time = time < s.meta.time ? s.meta.time : time
				time=(time-time_deb)/3600000
if (opts.distance_time) {dist_time=dist;} else {dist_time=time;}
                data.push({
                    dist: dist,
                    time: time,					
                    dist_time: dist_time,					
    ...
    _showDiagramIndicator: function(item, xCoordinate) {
        ...
        this._mousefocus.attr('x1', xCoordinate)
            ...
            if ((this._width()-xCoordinate)<35) {xdeca=84; } else {xdeca=0; }
			
			this._focusrectX.attr('x', xCoordinate+7-xdeca)
            .attr('y', -9)
            .attr('rx', 5)
            .attr('ry', 5)
            .attr('width', 70)
			...
			this._focusrectYZ.attr('x', xCoordinate+7-xdeca)
            .attr('y', this._height()-38)
            .attr('rx', 5)
            .attr('ry', 5)
            .attr('width', 70)
            .attr('height', 36)
...
;
        ...
        var alt = item.altitude,
            ...
            numX1 = opts.hoverNumber.formatter_time(time, opts.hoverNumber.decimalsX);

        if (opts.imperial) {
            if ((this._width()-xCoordinate)<35) {xCoordinate=xCoordinate-20; textanchor="end"} else { textanchor="start"}
            this._focuslabelX.attr("x", xCoordinate+10)
            ...
            this._focuslabelY.attr("y", this._height() - 5)
				...
				.style("fill", "#000")
                .text(numX + "mi");
            this._focuslabelZ.attr("y", this._height() - 25)
                .attr("x", xCoordinate+10)
                .style("text-anchor", textanchor)
				.style("fill", "#000")
                .text(numX1);
